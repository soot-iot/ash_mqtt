defmodule AshMqtt.Runtime.Transport.Test do
  @moduledoc """
  In-memory transport for tests. Records every publish/subscribe call
  on an Agent so the test can assert on it, and exposes `deliver/2` to
  simulate an incoming message from the broker.

  Owner-aware: `connect/2` records the test process's pid; `deliver/2`
  sends the inbound message to that pid as
  `{:ash_mqtt_msg, %Message{}}` — the same envelope the production
  transport uses, so the runtime client can't tell the difference.
  """
